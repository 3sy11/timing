...
import { Select, Empty, Spin, Card, Alert } from 'antd'
...
export default function LivePage() {
  ...
  useEffect(() => { fetchSymbols().then(setSymbols).catch(e => setErr(`获取品种列表失败: ${e.message}`)) }, [])

  const onSelect = async (val: string) => {
    setSelected(val); setErr('')
    const [symbol, interval] = val.split('|')
    setLoading(true)
    try {
      const [k, s] = await Promise.all([fetchKlines(symbol, interval), fetchSignals(symbol, interval)])
      const kArr = Array.isArray(k) ? k : []
      const sArr = Array.isArray(s) ? s : []
      setKlines(kArr); setSignals(sArr)
    } catch (e: any) { setErr(`加载数据失败: ${e.message}`); setKlines([]); setSignals([]) }
    setLoading(false)
  }
  ...
  return (
    <div style={{ padding: 16 }}>
      ...
      {!loading && selected && !klines.length && !err && <Empty description="该品种无数据" />}
    </div>
  )
}
