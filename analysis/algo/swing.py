...
def select_trend_leg(
    swings: List[SwingPoint], direction: Literal["up", "down", "latest"] = "latest", klines: Optional[List[Kline]] = None
) -> Optional[TrendLeg]:
    if not swings:
        return None
    if klines is None:
    ...
    while i < len(swings) - 1:
        ...
        if a.kind == "low" and b.kind == "high":
            ...
            if high > low:
                legs.append(TrendLeg(a.ts, b.ts, low, high, "up", a.index, b.index))
        elif a.kind == "high" and b.kind == "low":
            high, low = a.price, b.price
            if high > low:
                legs.append(TrendLeg(a.ts, b.ts, low, high, "down", a.index, b.index))
        i += 1
    if not legs:
        return None
    if direction == "up":
