...
class AnalysisEngine(AppService):
    ...
    commands = ["models"]
    subscribe = {
        "timing.CacheEngine.OnBar": OnBarForAnalysis,
        "timing.CacheEngine.OnDataIngested": OnCacheIngested,
    }

    def __init__(self, clock: Clock = None, touch_tolerance: float = 0.5,
                 touch_cooldown_sec: float = 60.0, protocol=None, router_mapping=None, subscribe=None, **kwargs):
    ...
    def set_fib_state(self, symbol: str, interval: str, levels: List[Tuple[float, float]],
                      leg_low: float = 0.0, leg_high: float = 0.0) -> None:
        key = (symbol, interval)
        self._fib_levels[key] = list(levels)
        self._detectors[key] = TouchDetector(cooldown_sec=self.touch_cooldown_sec, clock=self.clock)
