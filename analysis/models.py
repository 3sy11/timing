...
Command 通过 globals.app 调业务方法，CSV 落盘在 engine.recompute_fib 中完成。"""
import logging
...
class FibLinesUpdated(BaseEvent):
    ...
    levels: List[dict] = Field(default_factory=list); revision: int = 0
...
class FibLevelTouched(BaseEvent):
    ...
    symbol: str = ""; ratio: float = 0.0; level_price: float = 0.0; touch_price: float = 0.0; direction: str = "UNKNOWN"
    async def __call__(self, *args, **kwargs): return await super().__call__(*args, **kwargs)


class OnBarForAnalysis(BaseCommand):
    """subscriber：PushBars → 实时触线检测。"""
...
class OnCacheIngested(BaseCommand):
    """subscriber：OnDataIngested → 批量重算 fib + CSV 落盘。"""
    ...
    async def __call__(self, *args, **kwargs) -> Any:
        ...
        if not result:
            log.warning(f'[Analysis] fit_fib failed for {sym}/{interval}'); return {"error": "fit_fib no result"}
