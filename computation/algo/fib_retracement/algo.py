...
_FIT_RATIOS = [0.236, 0.382, 0.5, 0.618, 0.786]
_SCORE_RATIOS = [0.0, 0.236, 0.382, 0.5, 0.618, 0.786]
...
def _score_fit(prices, confs, high, low, direction, tol_pct=0.02):
    ...
    评分用 _SCORE_RATIOS (含 0.0): 验证外推的 0% 是否也有聚类对齐。
    覆盖率惩罚基于 6 条评分线的覆盖比例。
    """
    span = high - low
    if span <= 0:
    ...
    coverage = len(covered_ratios) / len(_SCORE_RATIOS)
    score *= coverage
        ...
        for j in range(i + 1, n):
            for ri in range(4):
                for rj in range(ri + 1, 5):
                    if direction == "up":
                        h, l = _solve_hl(prices[j], _FIT_RATIOS[ri],
                                         prices[i], _FIT_RATIOS[rj], direction)
                    else:
                        h, l = _solve_hl(prices[i], _FIT_RATIOS[ri],
                                         prices[j], _FIT_RATIOS[rj], direction)
                    if h <= l or l <= 0:
                        continue
