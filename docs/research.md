...
| **计算** | SQL/Python 批量计算 | klines | `indicators/`, `structures/` | `compute_id` |
| **分析** | 逐 bar 检测信号 | bar + structures | `signals/` | `analysis_id` |
| **决策** | 信号→交易决定 | signals | `decisions/` | `decision_id` |
| **执行** | 下单、成交、持仓 | decisions | `orders/`, `fills/`, `positions/` | `execution_id` |
...
| 表 | 主键 | 实验ID | 说明 |
|----|------|--------|------|
| klines | `symbol, interval, ts` | 无 | 原始数据，全局共享 |
| indicators | `symbol, interval, ts` | 无 | 公共衍生数据，固定参数，全局共享 |
| structures | `compute_id, symbol, interval, ts` | `compute_id` | 结构依赖算法参数，不同参数不同结果 |
| signals | `analysis_id, symbol, ts` | `analysis_id` | 同一份结构不同检测参数产出不同信号 |
| decisions | `decision_id, symbol, ts` | `decision_id` | 同一份信号不同策略参数产出不同决策 |
| orders | `execution_id, order_id, ts` | `execution_id` | 事件溯源：每次状态变更追加一行 |
| fills | `execution_id, order_id, ts` | `execution_id` | 成交记录 |
| positions | `execution_id, symbol, ts` | `execution_id` | 快照式（每次变动追加一行） |
...
| 表 | 自身ID | 追溯字段 | 含义 |
|----|--------|---------|------|
| structures | `compute_id` | — | 顶层 |
| signals | `analysis_id` | `compute_id` | 用了哪组 structure |
...
```
职责：从 klines 加工出 indicators 和 structures
触发：数据导入后 / 定时 / 手动 / 突破事件
产出：warehouse/indicators/{compute_id}/  warehouse/structures/{compute_id}/
...
| 类型 | 引擎 | 产出 | 示例 |
|------|------|------|------|
| SQL | DuckDB + talib | indicators | `ta_sma(close, 5)`, `ta_rsi(close, 14)` |
| Python | algo.py 纯函数 | structures | `compute_retracement(klines, cfg)` |
...
[computation.default.indicators]
# SQL 计算定义
...
[computation.default.structure]
# Python 计算定义
fn = "timing.algo.retracement.algo.compute_retracement"
config = { pivot_windows = [[5,5],[8,8]], zigzag_thresholds = [0.05, 0.10] }
```

**输出格式 — indicators**：

| compute_id | symbol | interval | ts | sma_5 | sma_20 | rsi_14 | atr_14 | ... |
|------------|--------|----------|-----|-------|--------|--------|--------|-----|
| base_v1 | 159363.OF | 1d | 1711152000 | 1.05 | 1.02 | 55.3 | 0.03 | ... |

**输出格式 — structures**：

| compute_id | symbol | interval | ts | groups_json |
|------------|--------|----------|-----|-------------|
| base_v1 | 159363.OF | 1d | 1711152000 | `[{leg: ..., levels: ...}]` |

### 4.4 分析（Analysis）

```
职责：接收 bar → 读取 structure → detect 信号
触发：新 bar 到达（实时）/ 回测批量
产出：warehouse/signals/{analysis_id}/
实验：analysis_id 标识检测参数 + 依赖的 compute_id
```
...
compute_id = "base_v1"           # 依赖哪次计算的 structure
symbol = "159363.OF"
interval = "1d"
...
| analysis_id | compute_id | symbol | ts | direction | strength | price | level | metadata |
|-------------|------------|--------|-----|-----------|----------|-------|-------|----------|
| ret_v1_tight | base_v1 | 159363.OF | 1711152000 | long | 0.85 | 1.05 | 1.03 | {...} |
...
│   ├── command.py              # ComputeIndicators, ComputeStructure — 绑定在 ComputationService 上
│   ├── indicators.py           # SQL 模板 + 执行逻辑
│   └── structure.py            # 调用 algo 纯函数 + 写 parquet
│
├── analysis/                    # ══ 分析服务 ══
│   ├── __init__.py
...
│   │                           #   on_bar → 读 structure → detect → 写 signals
│   ├── command.py              # 手动触发类命令（如 RerunDetect）
│   └── algo/                   # 纯函数（不是服务，无 app.py）
│       ├── __init__.py
│       ├── detect.py           # detect(bar, structure, state, cfg) → DetectResult
│       └── retracement/        # 回撤算法（不动）
│           ├── __init__.py
