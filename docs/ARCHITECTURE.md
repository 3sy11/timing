...
| Command（命令） | 任意发起方 → 目标 AppService | SubmitOrder, CancelOrder, PushBars, IngestParquetFile | 请求某个组件执行某个动作 |
...
触发点：手动 dispatch IngestParquetFile 命令，或回测启动时自动触发。

| **步骤** | **发起方** | **消息** | **接收方** | **结果** |
| --- | --- | --- | --- | --- |
| 1 | Runner / CLI | IngestParquetFile 命令 | DataEngine | 读取 Parquet 文件 |
...
| 阶段 1 （数据链路） | FileParquetDataClient DataEngine CacheEngine（含 OnBarReceived / OnDataIngested） | IngestParquetFile → CacheEngine.get\_klines() 返回正确数据 |
...
| timing/data/models.py | BarReceived, DataIngested, PushBars, IngestParquetFile | 引擎层 |
