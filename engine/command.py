...
class RunBacktest(BaseCommand):
    ...
    async def __call__(self, *args, **kwargs) -> Any:
        ...
        if not klines:
            log.warning(f'[回测] {symbol}/{interval} 无数据'); return None
        if len(klines) <= warmup_n:
            log.warning(f'[回测] K线数{len(klines)} ≤ warmup{warmup_n}，不足'); return None
        ...
        for i, bar in enumerate(replay_data):
            ...
            for svc in analysis_svcs:
                ...
                signal_rows = [{"symbol": symbol, "interval": interval, **sig} for sig in signals]
                await svc.db.append("signals", signal_rows)
                for sig in signals:
        ...
        for svc in analysis_svcs:
            if svc.db:
                sigs = await svc.db.get("signals", symbol=symbol, interval=interval)
        ...
        for svc in FibStrategy._apps.values():
            if hasattr(svc, 'db') and svc.db:
                decs = await svc.db.get("decisions", symbol=symbol)
                if decs: decisions.extend(decs)

        if broker.db:
            fills = await broker.db.all("fills")
