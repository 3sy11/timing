...
### Phase 1: 需求与发现
- [x] 理解用户意图：交易系统基础 + 首期斐波那契回撤与触线
- [x] 梳理 bollydog 能力与 timing 目录结构
- [x] 记录到 findings.md、确定架构
- **Status:** complete
...
- [x] 撰写 **timing/ARCHITECTURE.md**：整体架构 + 引擎层总览 + **行情接入层**（DataEngine、DataClient 端口、数据类型、Cache、数据流、目录与接口清单、检查项），参考 NautilusTrader 代码结构
...
### Phase 3: 实现（首期）
- [x] 实现 K 线模型与时间范围内数据获取（`timing/data`: Kline, ListKlineSource）
- [x] **实现 swing 识别**：`timing/analysis/swing` 中 find_swing_highs_lows、select_trend_leg，得到 (leg_low, leg_high)
- [x] 实现水平斐波那契回撤线计算：`timing/analysis/fibonacci` 中 retracement_from_leg、retracement_from_klines
- [x] 实现「实时价格 → 与回撤线比较 → 触线事件」逻辑：`timing/analysis/touch` 中 check_touch、TouchDetector（去抖）
...
### Phase 5: 交付
- [x] 代码就绪，可运行示例：`cd 项目根(3sy11) && python3 timing/example_fib_touch.py`
- [x] **使用 uv 管理依赖与虚拟环境**：项目根(3sy11) 下 `pyproject.toml`、`.python-version`、`.venv`；`uv venv` / `uv sync` / `uv run python timing/example_fib_touch.py`
