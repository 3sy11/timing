...
class FibStrategy(AppService):
    ...
    @property
    def broker(self):
        if isinstance(self.depends, dict) and "execution.Broker" in self.depends:
            return self.depends["execution.Broker"]
        return services.get("execution.Broker")
    ...
    async def on_signal(self, cmd):
        ...
        if direction == "neutral" or strength < self.min_strength:
            ...
            await self._record_decision(symbol, ts, direction, strength, price, "skip", reason); return
        ...
        broker = self.broker
        if broker:
            await broker.on_submit_order(symbol, side, "market", qty, price, 0.0, bar)
